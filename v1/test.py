...
# # sub_client.subscribe_candlestick_event("bchusdt", CandlestickInterval.MIN1, callback, error)
...
def get_historical_data():
    ...
    close:347.15
        ...
        volume:189.645
    '''
    klines = request_client.get_candlestick_data(symbol='BCHUSDT', interval='1m',limit=100)
    # # PrintBasic.print_obj(klines[-1])
    # last_n = klines[((-1) * 100):]
    # price_info['price_list'] = list()
    # price_info['latest_time'] = last_n[-1].openTime
    # for item in last_n:
    #     price_info['price_list'].append(item.close)
